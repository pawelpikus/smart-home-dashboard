...
const SideBar = ({ children, ...restProps }: Props) => {
  return (
    <div
      className="lg:row-start-2 bg-bgDarker lg:mx-0 lg:col-start-7 lg:col-span-2"
      {...restProps}
    >
      {children}
    </div>
...
SideBar.Container = function sideBarContainer({ children }: Props) {
  return (
    <div className="flex flex-col items-center justify-center w-11/12 gap-4 py-2 pb-24 mx-auto md:flex-row md:justify-between md:items-start">
      {children}
    </div>
  );
};
...
SideBar.Members = function SideBarMembers() {
  return (
    <div className="self-start w-full mt-6 md:w-1/2">
      <h3 className="mb-2 font-bold text-md lg:text-xl">At home now</h3>
      <div className="flex justify-center gap-2 py-2 bg-white rounded-lg md:flex-col md:items-center">
